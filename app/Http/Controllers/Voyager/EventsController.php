...
class EventsController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($dataType->server_side) {
            $searchNames = $dataType->browseRows->mapWithKeys(function ($row) {
                return [
                    $row['field'] => $row->getTranslatedAttribute(
                        'display_name'
                    ),
                ];
            });
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($search->value != '' && $search->key && $search->filter) {
                ...
                if (
                    $row = $this->findSearchableRelationshipRow(
            ...
            $row = $dataType->rows
                ->where('field', $orderBy)
                ->firstWhere('type', 'relationship');
            if (
                $orderBy &&
                (in_array($orderBy, $dataType->fields()) || !empty($row))
            ) {
            ...
            $dataTypeContent = $this->resolveRelations(
                $dataTypeContent,
                $dataType
            );
        } else {
    ...
    public function edit(Request $request, $id)
    {
        ...
        foreach ($dataType->editRows as $key => $row) {
            $dataType->editRows[$key]['col_width'] = isset($row->details->width)
                ? $row->details->width
                : 100;
        }
        ...
        return Voyager::view(
            $view,
            compact('dataType', 'dataTypeContent', 'isModelTranslatable')
        );
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $id =
            $id instanceof \Illuminate\Database\Eloquent\Model
                ? $id->{$id->getKeyName()}
                : $id;
    ...
    public function store(Request $request)
    {
        ...
        $event->save();
// Event::get()->last()->id;
        // $eventId = "eventcalendar";

        // Validate fields with ajax

        // $request->calendar_id = $eventId;
        ...
        if (!$request->has('_tagging')) {
            ...
            return $redirect->with([
                'message' =>
                    ...
                    " {$dataType->getTranslatedAttribute(
    ...
    public function destroy(Request $request, $id)
    {
        ...
        // $e->each(function ($collection) {
        //     if ($collection->summary == 'Test1') {
        ...
        // Get all events and delete the event with the same name based on its ID.
        ...
        $data = $res
            ? [
                'message' =>
                    __('voyager::generic.successfully_deleted') .
                    " {$displayName}",
                'alert-type' => 'success',
            ]
            : [
    ...
    public function restore(Request $request, $id)
    {
        ...
        $displayName = $dataType->getTranslatedAttribute(
            'display_name_singular'
        );
        ...
        $data = $res
            ? [
                'message' =>
                    __('voyager::generic.successfully_restored') .
                    " {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message' =>
                    __('voyager::generic.error_restoring') . " {$displayName}",
                'alert-type' => 'error',
            ];
    ...
    public function remove_media(Request $request)
    {
        try {
            ...
            if (!isset($data->{$field})) {
                throw new Exception(
                    __('voyager::generic.field_does_not_exist'),
                    400
                );
            }
            ...
            if (@json_decode($multi)) {
                ...
                if (is_null($key) || $key === false) {
                    throw new Exception(
                        __('voyager::media.file_does_not_exist'),
                        400
                    );
                }

                $fileToRemove =
                    $fieldData[$key]['download_link'] ?? $fieldData[$key];

                // Remove file from array
                unset($fieldData[$key]);

                // Generate json and update field
                $data->{$field} = empty($fieldData)
                    ? null
                    : json_encode(array_values($fieldData));
            } else {
    ...
    protected function cleanup($dataType, $data)
    {
        ...
        if (is_bread_translatable($data)) {
            $data->deleteAttributeTranslations(
                $data->getTranslatableAttributes()
            );
        }

        // Delete Images
        $this->deleteBreadImages(
            $data,
            $dataType->deleteRows->whereIn('type', ['image', 'multiple_images'])
        );
    ...
    public function deleteBreadImages($data, $rows, $single_image = null)
    {
        ...
        foreach ($rows as $row) {
            if ($row->type == 'multiple_images') {
                $images_to_remove =
                    json_decode($data->getOriginal($row->field), true) ?? [];
            } else {
            ...
            foreach ($images_to_remove as $image) {
                ...
                if (
                    $image != config('voyager.user.default_avatar') &&
                    (is_null($single_image) || $single_image == $image)
                ) {
    ...
    public function order(Request $request)
    {
        ...
        if (
            empty($dataType->order_column) ||
            empty($dataType->order_display_column)
        ) {
            return redirect()
                ->route("voyager.{$dataType->slug}.index")
                ->with([
        ...
        return Voyager::view(
            $view,
            compact('dataType', 'display_column', 'dataRow', 'results')
        );
    }
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type')) {
                ...
                if (
                    isset($options->scope) &&
                    $options->scope != '' &&
                    method_exists($model, 'scope' . ucfirst($options->scope))
                ) {
                ...
                if ($search) {
                    ...
                    if (in_array($options->label, $additional_attributes)) {
                        ...
                        $relationshipOptions = $relationshipOptions->forPage(
                            $page,
                            $on_page
                        );
                    } else {
                ...
                if (!empty($options->sort->field)) {
                    if (
                        !empty($options->sort->direction) &&
                        strtolower($options->sort->direction) == 'desc'
                    ) {
                        $relationshipOptions = $relationshipOptions->sortByDesc(
                            $options->sort->field
                        );
                    } else {
                        $relationshipOptions = $relationshipOptions->sortBy(
                            $options->sort->field
                        );
                    }
                }
    ...
    protected function findSearchableRelationshipRow(
        $relationshipRows,
        $searchKey
    ) {
